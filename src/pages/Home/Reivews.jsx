import { TbBrandAirbnb } from "react-icons/tb";
import { CiLocationArrow1 } from "react-icons/ci";
import { MdDesignServices } from "react-icons/md";
import { MdWeb } from "react-icons/md";
import { IoMdPlayCircle } from "react-icons/io";
import { Link } from "react-router-dom";
import CountUp from 'react-countup';



import { FaStar } from "react-icons/fa";

const Reivews = () => {
    return (
        <div className='bg-[#FFFFF5] pb-10 lg:px-32 pt-5 p-4'>
            <div className='border overflow-hidden'> </div>
            <h2 className='text-center  md:text-[60px] font-bold font-poppins text-4xl  leading-none lg:pt-20 pt-12 pb-10 lg:pb-28'> Clients are always <br/>satisfied with us
        <span> <img className='h-14 pt-3 inline' src="data:image/png;base64," alt="" /></span>   </h2>
       
       {/* Reiverws Area Part card */}
      {/* Card Area   */}
      <div className="pb-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                 {/* card 1 */}
                <div className='card  border-2 border-black p-4 mx-auto'>
                    <span className="text-3xl flex gap-1"> 
                        <FaStar></FaStar>  
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                    
                    </span> 
                     <div>
                        <h2 className="text-2xl pt-2 pb-1 font-bold">Super customer service!</h2>
                        <p className="text-md font-sans font-medium"> Excellent customer service and I was really impressed and happy with my purchase especially as it was a last minute order which got to me in time, and when it arrived I was very happy with the design and size and so was the recipient.</p>
                        <div className=" flex  items-center gap-4 mt-4">
                        <img className='w-16 h-16 rounded-full  border-black border-[3px]' src="https://aximo-react.vercel.app/assets/user1-DFxmj4MK.png" alt="" />
                        <span className="text-2xl font-sans font-semibold"> William Jack Founder@XYZ</span>
                         </div>
                     </div>
                </div>  
                
                {/* Card 2 */}
                <div className='card  border-2 border-black p-4 mx-auto'>
                    <span className="text-3xl flex gap-1"> 
                        <FaStar></FaStar>  
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                    
                    </span> 
                     <div>
                        <h2 className="text-2xl pt-2 pb-1 font-bold">Super customer service!</h2>
                        <p className="text-md font-sans font-medium"> Excellent customer service and I was really impressed and happy with my purchase especially as it was a last minute order which got to me in time, and when it arrived I was very happy with the design and size and so was the recipient.</p>
                        <div className=" flex  items-center gap-4 mt-4">
                        <img className='w-16 h-16 rounded-full  border-black border-[3px]' src="https://aximo-react.vercel.app/assets/user1-DFxmj4MK.png" alt="" />
                        <span className="text-2xl font-sans font-semibold"> William Jack Founder@XYZ</span>
                         </div>
                     </div>
                </div>  

           {/* Card 3 */}
           <div className='card  border-2 border-black p-4 mx-auto'>
                    <span className="text-3xl flex gap-1"> 
                        <FaStar></FaStar>  
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                    
                    </span> 
                     <div>
                        <h2 className="text-2xl pt-2 pb-1 font-bold">Super customer service!</h2>
                        <p className="text-md font-sans font-medium"> Excellent customer service and I was really impressed and happy with my purchase especially as it was a last minute order which got to me in time, and when it arrived I was very happy with the design and size and so was the recipient.</p>
                        <div className=" flex  items-center gap-4 mt-4">
                        <img className='w-16 h-16 rounded-full  border-black border-[3px]' src="https://aximo-react.vercel.app/assets/user1-DFxmj4MK.png" alt="" />
                        <span className="text-2xl font-sans font-semibold"> William Jack Founder@XYZ</span>
                         </div>
                     </div>
                </div>  

                 {/* Card 4 */}
                 <div className='card  border-2 border-black p-4 mx-auto'>
                    <span className="text-3xl flex gap-1"> 
                        <FaStar></FaStar>  
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                        <FaStar></FaStar>
                    
                    </span> 
                     <div>
                        <h2 className="text-2xl pt-2 pb-1 font-bold">Super customer service!</h2>
                        <p className="text-md font-sans font-medium"> Excellent customer service and I was really impressed and happy with my purchase especially as it was a last minute order which got to me in time, and when it arrived I was very happy with the design and size and so was the recipient.</p>
                        <div className=" flex  items-center gap-4 mt-4">
                        <img className='w-16 h-16 rounded-full  border-black border-[3px]' src="https://aximo-react.vercel.app/assets/user1-DFxmj4MK.png" alt="" />
                        <span className="text-2xl font-sans font-semibold"> William Jack Founder@XYZ</span>
                         </div>
                     </div>
                </div>   

            </div>
       
       
       
       
        </div>
    );
};

export default Reivews;